...
- **完整文档 PDF**：将正文单独分页为 A4 PDF，不受当前滚动位置或左右面板影响。
...
- 文件夹全文搜索结果现在不仅打开对应 Markdown 文件，还会根据源文件行号跳转到正文匹配位置、高亮关键词，并在左侧结果列表中保留当前选中状态；跨多个渲染文本节点的关键词也可被标记。
- 文档目录和页内搜索的远距离跳转改为瞬时到达目标附近、只在末端保留轻微平滑动作；近距离跳转仍保持自然过渡，并遵循系统“减少动态效果”设置。
- PDF 导出改用 MIT 许可的 `html2pdf.js`、`html2canvas` 和 `jsPDF` 生成完整 A4 文档；导出内容只包含正文，不再受当前滚动页、文件夹面板或文档目录遮挡影响。
- PDF 导出增加系统保存对话框、中英文导出状态、图片与字体等待，以及 `.pdf` 后缀和文件头写入校验。

**Bug 修复**

- 修复点击较远目录项时整篇文档长时间缓慢滚动的问题。
- 修复文件夹全文搜索只能打开文件、不能标记并定位正文匹配内容的问题。
- 修复 WebView 打印只能输出当前可见页，且打印内容可能被左右面板遮挡的问题。

> 本版已通过 102 项前端单元测试、Svelte 检查、生产构建、Rust 检查和 PDF 写入安全测试。完整 PDF 的分页、超长表格、远程图片和大文档内存占用仍需在 Windows 与 macOS 安装版上人工验收；基于 Canvas 的 PDF 主要保证渲染一致性，不保证所有文字都可选择复制。
...
- Windows 与 macOS 启动时，WebView 基础页面完成加载后立即显示应用窗口，不再把窗口出现时机绑定到历史文档恢复完成。
- 新增覆盖整个应用的主题启动遮罩，显示 BearAI Markdown LOGO、当前语言的产品名称、“正在恢复工作区…”状态和轻量进度动画；遮罩继承当前主题的 SVG/场景背景，并支持系统低动态模式。
- 当前文档可用后启动遮罩淡出，其他历史标签继续在后台恢复，减少历史文档较多时点击应用后长时间没有窗口反馈的问题。

**Bug 修复**

- 移除 macOS 窗口创建后重复切换原生 decorations 的运行时修复，原生交通灯、Overlay 标题栏、圆角和阴影继续由 macOS 专用 Tauri 配置负责，降低交通灯修复与首次显示相互干扰的风险。
- 移除前端历史恢复流程中的原生窗口 `show()` 调用，以及固定等待 5 秒才显示窗口的兜底线程；窗口显示和文档恢复现在是相互独立的状态。
- 增加启动窗口时机、主题遮罩和 macOS decorations 策略的回归测试。

> 本版已完成 Windows 单元测试、Svelte 检查、生产构建、Rust 检查与本地 Release/NSIS 构建。Windows 启动遮罩、macOS 首次/再次启动耗时、原生交通灯、圆角、全屏和后台会话恢复仍需在对应真实桌面系统上人工验收；CI 构建通过不能替代真实 Mac 验收。
...
- 标签页拖拽排序改为明确的“插入位置”交互：鼠标位于标签左半区时显示左侧插入线，位于右半区时显示右侧插入线；前一标签右侧与后一标签左侧共享同一个逻辑位置，并支持移动到标签栏最前和最后。
- 切换到已打开的本地文档时先立即激活对应标签和缓存内容，再在后台读取磁盘上的最新内容，减少内容空白后标签才切换的延迟感。
- 启动恢复会话时保持上次标签的准确顺序和当前文档，同时仍优先读取当前文档以缩短首屏等待。
- 点击目录树中的 Markdown 文件不再等待文件夹双击判定计时，文件打开响应更直接；文件夹仍保留双击重命名行为。

**Bug 修复**

- 修复切换标签后无法取得文件在外部发生的最新修改的问题；编辑中或包含未保存修改的标签不会被自动刷新覆盖。
- 修复较早发起的异步刷新晚于当前标签完成时，可能把当前阅读器内容错误替换成旧标签内容的竞态问题。
- 修复恢复历史文档时为了优先加载当前文档而把它移动到标签栏第一项、导致原有顺序和当前项不能同时保留的问题。
- 修复标签拖拽反馈始终显示在目标标签左侧、无法区分插入到标签之前还是之后的问题。

> 本版已完成前端单元测试、Svelte 检查、生产构建和 Rust/Tauri 本地门禁；Windows 与 macOS 安装包由 GitHub Actions 构建。拖拽手感、真实会话重启恢复和 macOS 窗口行为仍应在对应桌面系统上人工验收。
...
2. 在真实 macOS 设备上验收主题启动遮罩、首次与再次启动耗时、原生交通灯位置、悬停符号、圆角、全屏、系统确认删除、文件/文件夹删除、系统最近文档、安装、签名与
...
完整文档 PDF 导出由 [html2pdf.js](https://github.com/eKoopmans/html2pdf.js/)、
[html2canvas](https://github.com/niklasvh/html2canvas) 与 [jsPDF](https://github.com/parallax/jsPDF)
提供支持；这些项目均采用 MIT 许可。
